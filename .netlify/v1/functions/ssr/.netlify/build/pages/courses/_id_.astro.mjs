...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { data: courseExams } = await supabase.from("exams").select("*").eq("class_id", id).eq("is_published", true);
  const courseModules = sortedCourseModules || [];
  let isCourseEnrolledSSR = false;
  let completedMaterialIds = [];
  let purchasedModuleIdList = [];
  if (user) {
    const [enrollResult, progressResult, modEnrollResult] = await Promise.all([
      supabase.from("enrollments").select("id").eq("user_id", user.id).eq("class_id", id).maybeSingle(),
      supabase.from("progress").select("material_id").eq("user_id", user.id).eq("completed", true),
      supabase.from("module_enrollments").select("module_id").eq("user_id", user.id)
    ]);
    isCourseEnrolledSSR = !!enrollResult.data;
    completedMaterialIds = (progressResult.data || []).map((p) => p.material_id);
    purchasedModuleIdList = (modEnrollResult.data || []).map((e) => e.module_id);
  }
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "data-astro-cid-ae7xwjmx": true }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div id="user-data"${addAttribute(JSON.stringify(user || null), "data-user")}${addAttribute(isCourseEnrolledSSR ? "true" : "false", "data-enrolled")}${addAttribute(JSON.stringify(completedMaterialIds), "data-completed-ids")}${addAttribute(JSON.stringify(purchasedModuleIdList), "data-purchased-module-ids")} class="hidden" data-astro-cid-ae7xwjmx></div> <div class="course-header" data-astro-cid-ae7xwjmx> <h1 data-astro-cid-ae7xwjmx>${course.title}</h1> <p class="description" data-astro-cid-ae7xwjmx>${course.description}</p> <div class="meta" data-astro-cid-ae7xwjmx> <span class="price" data-astro-cid-ae7xwjmx>${course.price && course.price > 0 ? `Biaya Masuk: Rp ${course.price.toLocaleString("id-ID")}` : "Gratis"}</span> <button id="enroll-btn" class="btn btn-primary"${addAttribute(id, "data-course-id")}${addAttribute(course.price || 0, "data-course-price")} data-astro-cid-ae7xwjmx>
Start Learning
